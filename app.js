...
function persist(/*key – kept for compat, ignored*/) {
  FS_DOC().set({
    ...
    revenueExpenses: state.revenueExpenses,
...
async function fetchState() {
  try {
    ...
    state.revenueExpenses = data.revenueExpenses || {};
  } catch(e) {
...
function closeModal() {
  ...
  // Reset any per-modal body styles that viewDoc() may have set
...
/* ─────────────────────────────────────────────
   11. NAVIGATION
───────────────────────────────────────────── */
const VIEW_TITLES = { dashboard:'Dashboard', invoices:'Invoices', quotations:'Quotations', revenue:'Revenue', items:'Items / Products', customers:'Customers', settings:'Settings' };
...
function renderView(view) {
  ...
  else if (view === 'revenue')    { renderRevenueView(vc, ta); }
...
function docFormHTML(type, doc={}) {
  ...
  <div class="form-row">
    ...
    <div class="form-group">
      <label>Branch</label>
      <input type="text" id="doc-branch" placeholder="e.g. Lagos, Abuja…" value="${esc(doc.branch||'')}" />
    </div>
  </div>
...
function collectDocForm(type, existing={}) {
  ...
  return {
    ...
    branch:   document.getElementById('doc-branch')?.value.trim() || '',
...
function buildDocHTML(doc, type) {
  ...
  const dateLabel2 = isInv ? 'DUE DATE' : 'EXPIRY DATE';
  ...
  return `<div class="oi-doc-wrap"><div class="oi-doc">
    ...
    <div class="oi-meta-row">
      ...
      <div class="oi-docinfo">
        ...
        <div class="oi-docinfo-row">
          <span class="oi-di-label">${dateLabel2}</span>
          <span class="oi-di-val">${fmtDate(date2)}</span>
        </div>
        ...
        ${doc.branch ? `<div class="oi-docinfo-row"><span class="oi-di-label">BRANCH</span><span class="oi-di-val">${esc(doc.branch)}</span></div>` : ''}
      </div>
    </div>
...
window.viewDoc = function(id, type) {
  ...
  openModal({
    ...
    footer: `<button class="btn btn-secondary" onclick="closeModal()">Close</button>
             <button class="btn btn-primary" id="btn-send-email" style="background-color: var(--accent-blue);" onclick="sendEmail('${id}', '${type}')">
...
window.sendEmail = async function(id, type) {
  const doc = type==='invoice' ? state.invoices.find(x=>x.id===id) : state.quotations.find(x=>x.id===id);
  if (!doc) return;
  if (!doc.client || !doc.client.email) {
    toast('Client does not have an email address.', 'error');
    return;
  }
  if (!state.settings.smtp || !state.settings.smtp.host) {
    toast('Please configure SMTP settings first in the Settings tab.', 'error');
    return;
  }
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = `<svg class="spinner" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation: spin 1s linear infinite;"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg> Sending...`;
  }

  try {
    // 1. Generate the raw HTML payload for the backend to render via Puppeteer
    const printHtml = buildPrintHTML(doc, type);

    // 2. Prepare Email Body
    const htmlBody = `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
        <h2>${state.settings.company || 'Your Company'}</h2>
        <p>Hello ${doc.client.name},</p>
        <p>Please find attached the <strong>${type === 'invoice' ? 'Invoice' : 'Quotation'} (${doc.number})</strong>.</p>
        <p><strong>Total Amount:</strong> ${fmtMoney(doc.total || 0)}</p>
        <p>${state.settings.footer || 'Thank you for your business!'}</p>
      </div>
    `;
    ...
    const res = await fetch('/api/send-email', {
      ...
      body: JSON.stringify({
        to: doc.client.email,
        subject: `${state.settings.company || 'Your Company'} - ${type === 'invoice' ? 'Invoice' : 'Quotation'} ${doc.number}`,
        html: htmlBody,
        printHtml: printHtml,
    ...
    // 4. Update status to 'Sent' and persist
    doc.status = 'sent';
    persist();
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 2L11 13"/><path d="M22 2l-7 20-4-9-9-4 20-7z"/></svg> Send Email`;
    }
  }
};
...
function renderSettingsView(vc, ta) {
  ...
  <div class="settings-grid">
    ...
    <div class="card">
      <h3 style="font-size:0.95rem;font-weight:700;margin-bottom:16px">SMTP Settings (For sending emails)</h3>
      <p style="font-size:0.83rem;color:var(--text-secondary);margin-bottom:16px">Configure your SMTP server to send invoices and quotations directly from the app.</p>
      <div class="form-row">
        <div class="form-group">
          <label>SMTP Host</label>
          <input type="text" id="s-smtp-host" value="${esc(s.smtp?.host||'')}" placeholder="e.g. smtp.gmail.com" />
        </div>
        <div class="form-group">
          <label>SMTP Port</label>
          <input type="number" id="s-smtp-port" value="${s.smtp?.port||'587'}" />
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>SMTP Username</label>
          <input type="text" id="s-smtp-user" value="${esc(s.smtp?.user||'')}" placeholder="e.g. you@example.com" />
        </div>
        <div class="form-group">
          <label>SMTP Password / App Password</label>
          <input type="password" id="s-smtp-pass" value="${esc(s.smtp?.pass||'')}" />
        </div>
      </div>
      <div class="form-group">
        <label>From Email Address</label>
        <input type="email" id="s-smtp-from" value="${esc(s.smtp?.from||'')}" placeholder="e.g. billing@yourcompany.com" />
      </div>
    </div>
...
window.saveSettings = function() {
  state.settings = {
    ...
    smtp: {
      ...
      port: document.getElementById('s-smtp-port')?.value.trim() || '587',
      user: document.getElementById('s-smtp-user')?.value.trim() || '',
      pass: document.getElementById('s-smtp-pass')?.value.trim() || '',
...
/* ─────────────────────────────────────────────
   21. REVENUE VIEW
───────────────────────────────────────────── */
const getBranch = inv => (inv.branch || '').trim() || 'Unassigned';

let _revExpenseTimer = null;

window.setRevenueExpense = function(id, val) {
  state.revenueExpenses[id] = parseFloat(val) || 0;
  updateRevenueRow(id);
  updateAllRevenueTotals();
  clearTimeout(_revExpenseTimer);
  _revExpenseTimer = setTimeout(() => persist(), 800);
};

function updateRevenueRow(id) {
  const inv = state.invoices.find(x => x.id === id);
  if (!inv) return;
  const total   = inv.total || 0;
  const wht     = total * 0.02;
  const vat     = total * 0.075;
  const expense = state.revenueExpenses[id] || 0;
  const profit  = total - wht - vat - expense;
  const el = document.querySelector(`.rev-profit[data-id="${id}"]`);
  if (el) {
...
function updateAllRevenueTotals() {
  const activeFilter = state.revenueFilter || 'all';

  document.querySelectorAll('.rev-branch-group').forEach(group => {
    let bTotal = 0, bWHT = 0, bVAT = 0, bExpense = 0, bProfit = 0;
    group.querySelectorAll('.rev-row').forEach(row => {
      ...
      const vat     = total * 0.075;
      const profit  = total - wht - vat - expense;
      bTotal += total; bWHT += wht; bVAT += vat; bExpense += expense; bProfit += profit;
    });
    const btRow = group.querySelector('.rev-branch-total');
    if (btRow) {
      btRow.querySelector('.bt-total').textContent   = fmtMoney(bTotal);
      btRow.querySelector('.bt-wht').textContent     = fmtMoney(bWHT);
      btRow.querySelector('.bt-vat').textContent     = fmtMoney(bVAT);
      btRow.querySelector('.bt-expense').textContent = fmtMoney(bExpense);
      const bpEl = btRow.querySelector('.bt-profit');
      bpEl.textContent = fmtMoney(bProfit);
      bpEl.style.color = bProfit >= 0 ? 'var(--accent-green)' : 'var(--accent-red)';
    }
  });

  let gTotal = 0, gWHT = 0, gVAT = 0, gExpense = 0, gProfit = 0;
  state.invoices.forEach(inv => {
    if (activeFilter !== 'all' && getBranch(inv) !== activeFilter) return;
    const total   = inv.total || 0;
    const expense = state.revenueExpenses[inv.id] || 0;
    const wht     = total * 0.02;
    const vat     = total * 0.075;
    const profit  = total - wht - vat - expense;
    gTotal += total; gWHT += wht; gVAT += vat; gExpense += expense; gProfit += profit;
  });

  const $ = cls => document.querySelector(`#rev-grand-totals .${cls}`);
  if ($('gt-total')) {
    $('gt-total').textContent   = fmtShort(gTotal);
    $('gt-wht').textContent     = fmtShort(gWHT);
    $('gt-vat').textContent     = fmtShort(gVAT);
    $('gt-expense').textContent = fmtShort(gExpense);
    const gpEl = $('gt-profit');
    gpEl.textContent = fmtShort(gProfit);
    gpEl.style.color = gProfit >= 0 ? 'var(--accent-green)' : 'var(--accent-red)';
  }
}

function renderRevenueView(vc, ta) {
  const allBranches = [...new Set(state.invoices.map(getBranch))].sort((a, b) => {
  ...
  const filteredBranches = activeFilter === 'all'
    ? allBranches
    : allBranches.filter(b => b === activeFilter);

  let gTotal = 0, gWHT = 0, gVAT = 0, gExpense = 0, gProfit = 0;

  const branchSections = filteredBranches.map(branch => {
    const invoices = state.invoices.filter(i => getBranch(i) === branch);
    let bTotal = 0, bWHT = 0, bVAT = 0, bExpense = 0, bProfit = 0;
    ...
    const rows = invoices.map(inv => {
      ...
      const profit  = total - wht - vat - expense;
      bTotal += total; bWHT += wht; bVAT += vat; bExpense += expense; bProfit += profit;
      return `<tr class="rev-row" data-id="${inv.id}">
        <td><span class="doc-number-chip">${esc(inv.number)}</span></td>
        <td>${esc(inv.client?.name || '—')}</td>
        <td>${fmtDate(inv.date)}</td>
        <td><span class="badge badge-${inv.status}">${inv.status}</span></td>
        <td style="text-align:right;font-weight:600">${fmtMoney(total)}</td>
        <td class="rev-wht-cell" style="text-align:right;color:var(--accent-amber)">${fmtMoney(wht)}</td>
        <td class="rev-vat-cell" style="text-align:right;color:var(--accent-blue)">${fmtMoney(vat)}</td>
        <td class="rev-expense-cell">
          <input type="number" class="rev-expense-input" min="0" step="any"
            value="${expense || ''}" placeholder="0.00"
            oninput="setRevenueExpense('${inv.id}', this.value)" />
        </td>
        <td class="rev-profit" data-id="${inv.id}"
          style="text-align:right;font-weight:700;color:${profit >= 0 ? 'var(--accent-green)' : 'var(--accent-red)'}">
          ${fmtMoney(profit)}
        </td>
      </tr>`;
    }).join('');

    gTotal += bTotal; gWHT += bWHT; gVAT += bVAT; gExpense += bExpense; gProfit += bProfit;

    return `<div class="rev-branch-group" data-branch="${esc(branch)}">
      <div class="rev-branch-header">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
        <span class="rev-branch-name">${esc(branch)}</span>
        <span class="rev-branch-count">${invoices.length} invoice${invoices.length !== 1 ? 's' : ''}</span>
      </div>
      <div class="table-wrap rev-table-wrap">
        <table>
          <thead><tr>
            ...
            <th style="text-align:right">WHT (2%)</th>
            <th style="text-align:right">VAT (7.5%)</th>
            <th style="text-align:right">Expense</th>
            <th style="text-align:right">Actual Profit</th>
          </tr></thead>
          <tbody>
            ${rows || '<tr><td colspan="9"><div class="empty-state" style="padding:16px"><p>No invoices in this branch</p></div></td></tr>'}
            <tr class="rev-branch-total">
              <td colspan="4" class="rev-bt-label">Branch Total</td>
              <td class="bt-total rev-num-cell" style="color:var(--text-primary)">${fmtMoney(bTotal)}</td>
              <td class="bt-wht  rev-num-cell" style="color:var(--accent-amber)">${fmtMoney(bWHT)}</td>
              <td class="bt-vat  rev-num-cell" style="color:var(--accent-blue)">${fmtMoney(bVAT)}</td>
              <td class="bt-expense rev-num-cell">${fmtMoney(bExpense)}</td>
              <td class="bt-profit  rev-num-cell"
                style="color:${bProfit >= 0 ? 'var(--accent-green)' : 'var(--accent-red)'}">
                ${fmtMoney(bProfit)}
              </td>
            </tr>
          </tbody>
  ...
  /* ── Filter bar ── */
  const filterBar = `<div class="filter-bar" style="margin-bottom:24px">
    <button class="filter-btn${activeFilter === 'all' ? ' active' : ''}"
      onclick="setRevenueFilter('all')">All Branches</button>
    ${allBranches.map(b =>
      `<button class="filter-btn${activeFilter === b ? ' active' : ''}"
        onclick="setRevenueFilter(this.dataset.branch)"
        data-branch="${esc(b)}">${esc(b)}</button>`
    ).join('')}
  </div>`;
  ...
  const emptyState = `<div class="empty-state">
    ...
    <p>Create invoices and assign them to a branch to see revenue analysis here.</p>
  </div>`;

  /* ── Grand totals (same KPI cards as Dashboard) ── */
  const invCount = activeFilter === 'all'
    ? state.invoices.length
    : state.invoices.filter(i => getBranch(i) === activeFilter).length;
  const grandTotals = `<div class="kpi-grid" id="rev-grand-totals" style="margin-top:36px;padding-top:28px;border-top:2px solid var(--border)">
    <div class="kpi-card indigo">
      <div class="kpi-label">Total Revenue</div>
      <div class="kpi-value gt-total">${fmtShort(gTotal)}</div>
      <div class="kpi-sub">${invCount} invoice${invCount !== 1 ? 's' : ''}</div>
    </div>
    <div class="kpi-card amber">
      <div class="kpi-label">Total WHT (2%)</div>
      <div class="kpi-value gt-wht">${fmtShort(gWHT)}</div>
      <div class="kpi-sub">Withheld tax</div>
    </div>
    <div class="kpi-card blue">
      <div class="kpi-label">Total VAT (7.5%)</div>
      <div class="kpi-value gt-vat">${fmtShort(gVAT)}</div>
      <div class="kpi-sub">Value added tax</div>
    </div>
    <div class="kpi-card" style="border-color:var(--border)">
      <div class="kpi-label">Total Expense</div>
      <div class="kpi-value gt-expense">${fmtShort(gExpense)}</div>
      <div class="kpi-sub">Operational cost</div>
    </div>
    <div class="kpi-card green">
      <div class="kpi-label">Total Actual Profit</div>
      <div class="kpi-value gt-profit" style="color:${gProfit >= 0 ? 'var(--accent-green)' : 'var(--accent-red)'}">${fmtShort(gProfit)}</div>
      <div class="kpi-sub">After all deductions</div>
    </div>
  </div>`;

  vc.innerHTML = state.invoices.length === 0
    ? filterBar + emptyState
    : filterBar + branchSections + grandTotals;
}

window.setRevenueFilter = function(branch) {
  state.revenueFilter = branch;
  renderRevenueView(document.getElementById('view-container'), document.getElementById('topbar-actions'));
};
