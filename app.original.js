...
/* ─────────────────────────────────────────────
   11. NAVIGATION
───────────────────────────────────────────── */
const VIEW_TITLES = { dashboard:'Dashboard', invoices:'Invoices', quotations:'Quotations', items:'Items / Products', settings:'Settings' };
...
window.printDoc = function(id, type) {
  ...
  area.innerHTML = buildDocHTML(doc, type);
  window.print();
};
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ── Sidebar collapse toggle (stays in localStorage – it's per-user UI pref) ──
  const sidebarToggleBtn = document.getElementById('sidebar-toggle-btn');
  const applyCollapsed = (collapsed) => {
    document.body.classList.toggle('sidebar-collapsed', collapsed);
    localStorage.setItem('ig_sidebar_collapsed', collapsed ? '1' : '0');
  };
  if (localStorage.getItem('ig_sidebar_collapsed') === '1') applyCollapsed(true);
  ...
  setInterval(async () => {
    ...
    await fetchState();
    // Only re-render if no modal is open (avoids disrupting active editing)
